...
from seq2seq.beam_new import BeamSearch, BeamSearchNode
...
def main(args):
    ...
    for i, sample in enumerate(progress_bar):
        ...
        for _ in range(args.max_len - 1):
            # Get current nodes from beam searches
            all_nodes = [n[1] for s in searches for n in s.get_current_beams()]

            # Separate completed and non-completed nodes to avoid generating conflicts with finished ones
            completed_nodes = [node for node in all_nodes if node.is_complete]
            nodes = [node for node in all_nodes if not node.is_complete]

            #adding the completed translations back to the beam search to include in pruning
            ...
            for i in range(log_probs.shape[0]):
                for j in range(args.beam_size):
                    ...
                    # Add the node to current nodes for next iteration
                        ...
                        # print(f"Current sequence: {node.sequence}")
